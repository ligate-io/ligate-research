...
**Ligate Labs Research, Working Paper v0.2.1**
...
The reference simulator at [`prototypes/time-locked-attestations-sim/`](../../prototypes/time-locked-attestations-sim/) shipped with v0.2.1 (M1 milestone). It validates the cryptographic and mechanism layers of the paper. The §6 use-case-validated work (strategic-bidder sealed-bid auction game, embargo-leak game) is explicitly **out of M1 scope** and lands when the §6.1 design-partner gate opens.

**Shipped modules under `src/time_locked_attestations_sim/`:**

- `commitment.py`: §3.1 `Commitment` tuple, §3.4 hash-function dispatch (SHA-256 default, BLAKE3 with optional native dispatch, Poseidon signaled-but-not-implemented per §9.1).
- `lifecycle.py`: §3.3 four-state machine (COMMITTED, REVEALED, EXPIRED, CLEANED-UP) with deterministic transition rules.
- `transactions.py`: §4.1 / §4.2 / §4.3 admission-time checks for `MsgCommit`, `MsgReveal`, `MsgCleanup`, plus §4.5 batched-reveal sequencing.
- `failure_modes.py`: §8 attack-scenario harnesses (never-reveal, late-reveal, front-running, hash collisions, nonce reuse, reveal-DoS) returning structured `FailureModeResult` records with quantitative bounds.

**Shipped test coverage (41 tests, all passing):**

- §3.3 lifecycle state-machine transitions, including terminal-state persistence
- §3.4 nonce-floor enforcement (rejects nonces below 128 bits)
- §4.1 / §4.2 / §4.3 admission-time checks with tagged rejection reasons for mempool diagnostics
- §4.5 block-sequencing rule (reveals before cleanups before everything else)
- §5.1 binding via collision-distinct-input test
- §5.3 nonce-entropy bound: 128-bit floor keeps even N = 1000 reuses at ≈118 bits effective security
- §5.4 time-lock via reveal-before-`reveal_at` and reveal-after-TTL both rejected
- §8.1 never-reveal: deterministic transition to EXPIRED
- §8.5 nonce-reuse: quantitative bit-security degradation as a function of N
- §8.6 reveal-DoS: economic-bound assertion under deposit and cleanup pricing

**Cross-language test vectors** at [`test_vectors/commitment_canonical_encoding.json`](../../prototypes/time-locked-attestations-sim/test_vectors/commitment_canonical_encoding.json) carry four canonical SHA-256 commitment hashes (binary YES/NO, sealed-bid integer, long embargoed text) for downstream Rust / TypeScript verification. Format matches the convention of `per-schema-fees-sim/test_vectors/` and `native-delegation-sim/test_vectors/`.

**Headline figure** [`out/failure_modes_panel.png`](../../prototypes/time-locked-attestations-sim/out/failure_modes_panel.png): a 2×3 panel summarizing each §8 failure mode's quantitative bound. Generated by [`scripts/run_failure_mode_panel.py`](../../prototypes/time-locked-attestations-sim/scripts/run_failure_mode_panel.py).

**What M1 does not cover.** The simulator stops at the cryptographic and mechanism layers. Strategic-bidder game-theory simulation (sealed-bid auction with N agents, valuation distributions, reveal-or-default equilibrium) is M2 territory and gated on §6.1 design-partner validation. The decision to ship M1 ahead of the §6 gate reflects that the cryptographic and mechanism claims are partner-independent; the auction and embargo game theory is not.
