...
- **Hybrid post-quantum signatures.** Specified in the companion [PQ Migration](https://github.com/ligate-io/ligate-research/tree/main/papers/pq-migration) paper. The mechanism extends Lemma 1's primitive-agnosticism property: PoUA's economic-security argument survives a crypto swap intact (Appendix A of that paper). Concrete migration targets are NIST FIPS 204 (ML-DSA) and FIPS 205 (SLH-DSA).
...
**Real-chain calibration (v0.10).** The synthetic $\alpha \in \{2.0, 2.5, 3.0\}$ figure above quantifies the gap structurally but uses chosen power-law exponents rather than an exponent fitted to a real chain. v0.10 closes that empirical gap by pulling a recent 50-block window from Ethereum mainnet (12,885 transactions, blocks 25,194,472-25,194,521, sourced via a free public RPC and cached at [`prototypes/poua-sim/data/`](https://github.com/ligate-io/ligate-research/tree/main/prototypes/poua-sim/data/)), projecting to the (sender, recipient) bipartite, and fitting a power-law tail via the Hill estimator. The fitted exponents are $\hat{\alpha}_{\text{sender}} = 2.26$ and $\hat{\alpha}_{\text{recipient}} = 1.95$; both sides admit heavy tails (KS p-value vs an exponential null $\approx 0$ on both sides, sample sizes 6,930 and 4,131 unique addresses). The empirical $\hat{\alpha}$ sits at the heavy-tailed end of the synthetic range used in Figure \ref{fig:a3-fpr-comparison}. Figure \ref{fig:a3-fpr-realchain} reruns the A3 FPR Monte Carlo using the empirical degree sequence (resampled with replacement to the same $30 \times 30$ bipartite size) and confirms the structural finding: under realistic chain-graph degrees, realized FPR drops 3-30$\times$ below the analytical $\beta_3 = 1\%$ target, consistent with the synthetic-$\alpha$ result. This strengthens but does not close the §A.4 calibration gap: closing it requires Ligate's own devnet attestation traffic (mid-2026) so the threshold can be re-derived against the chain's domestic degree distribution rather than a foreign-chain structural proxy. The honest framing of the v0.10 empirical work is "the gap exists under real chain-graph data, not just under synthetic power-law data;" the production-calibration step remains open. Reproducible via [`scripts/fetch_ethereum_bipartite.py`](https://github.com/ligate-io/ligate-research/tree/main/prototypes/poua-sim/scripts/fetch_ethereum_bipartite.py), [`scripts/fit_powerlaw_and_calibrate.py`](https://github.com/ligate-io/ligate-research/tree/main/prototypes/poua-sim/scripts/fit_powerlaw_and_calibrate.py), and [`scripts/run_a3_fpr_realchain.py`](https://github.com/ligate-io/ligate-research/tree/main/prototypes/poua-sim/scripts/run_a3_fpr_realchain.py).

\begin{figure}[h]
\centering
\includegraphics[width=0.92\textwidth]{../../prototypes/poua-sim/out/a3_fpr_realchain.png}
\caption{A3 false-positive rate under the §A.2 Erdős-Rényi null versus a Chung-Lu null using the empirical degree distribution from an Ethereum mainnet sample (12,885 transactions, blocks 25,194,472-25,194,521, sourced 2026-05-28 via a free public RPC; 6,930 unique senders, 4,131 unique recipients). 5,000 Monte Carlo trials per $p_{\text{base}} \in \{0.02, 0.05, 0.10, 0.15, 0.20\}$ on a $30 \times 30$ bipartite graph; senders and recipients resampled with replacement from the empirical sequences. The Hill estimator on the cached degree sequences gives $\hat{\alpha}_{\text{sender}} = 2.26$ and $\hat{\alpha}_{\text{recipient}} = 1.95$ (both sides KS-rejected against an exponential null). Under the ER null the realized FPR tracks nominal within $\sim$2$\sigma$ binomial. Under the empirical Chung-Lu null the realized FPR sits 3-30$\times$ below nominal across the swept range. This confirms the §A.4 structural finding (Figure \ref{fig:a3-fpr-comparison}) with real chain-graph data rather than synthetic $\alpha$; production calibration against Ligate's own devnet traffic (mid-2026) is still required.}
\label{fig:a3-fpr-realchain}
\end{figure}
...
*End of working paper v0.10. Comments welcome to hello@ligate.io.*

*Roadmap: v0.10 (this version, 2026-05-28) shipped the §A.4 real-chain Chung-Lu calibration ([issue #120](https://github.com/ligate-io/ligate-research/issues/120) closed) by fitting a power-law tail to a 12,885-transaction Ethereum mainnet sample and confirming the synthetic-$\alpha$ finding under empirical degrees. v1.0 will incorporate Ligate devnet calibration data once devnet stabilizes (mid-2026), reviewer feedback from the arXiv submission, and any §A.3 / §A.4 production-calibration refinements driven by domestic chain-graph baselines. v1.0 is the venue-submission target (mid-2027 if appropriate venue).*
