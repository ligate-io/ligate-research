...
- **Wallet, relayer, MCP server, verifier registry.** These are the products on top of the protocol (Mneme, Iris, Themisra, Atlas respectively). Their architectures are out of paper scope; the paper only specifies the consensus layer the products run on.
- **Tokenomics specifics.** The `$AVOW` total supply (1B fixed), per-attestation fee level, and burn destination are mentioned for context but the paper does not derive the optimal tokenomics. v0 parameter recommendations in §7 are calibration targets, not theorems.
